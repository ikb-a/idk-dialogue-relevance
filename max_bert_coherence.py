...
"""Just compute cosine similarity of MAX pooled embeddings under BERT"""
def main(dataset, load_test_set):
    if not load_test_set:
        save_dir = "exp/BERT_max_coh"+dataset
    else:
        save_dir = "exp/BERT_max_coh_test_"+dataset
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(tqdm(val_loader)):
            ...
            feats1, _ = bert_model(input_ids=tok1, attention_mask=att1, token_type_ids = ty1)
            feats2, _ = bert_model(input_ids=tok2, attention_mask=att2, token_type_ids = ty2)

            feats1, _ = torch.max(feats1, dim=1)
            feats2, _ = torch.max(feats2, dim=1)
