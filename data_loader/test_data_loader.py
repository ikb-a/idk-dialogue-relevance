"""
Ugly hack, same code, but with test in place of val.
...
ROOT = 'data/'
ROOT2 = 'data/'
# Map enum to train, val, test set paths (NOTE: HACK; test in place of val)
DATASET_PATHS = {
    QualDataset.HUMOD: (None, ROOT2+'humod/test_HUMOD.json', None),
    QualDataset.USR_TC_REL: (None, ROOT2 + 'usr_tc/usr_tc_rel_test.json', None),
    QualDataset.P_DD: (None, ROOT2 + 'pang_dd/pang_dd.json', None),
    QualDataset.FED_COR: (None, ROOT2 + 'fed/fed_cor.json', None),
    QualDataset.FED_REL: (None, ROOT2 + 'fed/fed_rel.json', None)
}
...
class DataLoaderGenerator:
    ...
    def __init__(self, bert_max_len: int, to_load: QualDataset, load_type: LoaderType, batchsize: int, skip_tr=False, limit=None) -> None:
        ...
        if not skip_tr:
            if load_type in [LoaderType.Distr0, LoaderType.IDK, LoaderType.Rand3750]:
                self.tr = [tr_X1, tr_X2]
            else:
                self.tr = [tr_X1]
    ...
    def get_train_loader(self) -> DataLoader:
        ...
        elif self.load_type in [LoaderType.Distr0, LoaderType.IDK, LoaderType.Rand3750]:
            if self.tr_loader is None:
