...
if __name__ == '__main__':
    ...
    brown_dataset = brown.sents()
    brown_dataset = [' '.join(sent) for sent in brown_dataset]

    sequential_data = brown_dataset
    distractors = brown_dataset
    ...
    for i in range(len(sequential_data) - 1):
        ...
        if i % 100 == 0:
            print(f"({i}/{len(brown_dataset)}) ORI ACC: {acc_full/total}")
            print(f"({i}/{len(brown_dataset)}) Masked ACC: {acc_masked/total}")
    print("==================================")
    print("DONE RUNNING!")
