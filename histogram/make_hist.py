...
if __name__ == '__main__':
    load_dir = 'exp/HUMOD_IDK_bce_L1_wt1_lim3750'
    repeat = '0'
    epoch_target = 2
    ...
    plt.savefig('humod_idk_l1_bce_weights.jpg')
    plt.close()
    plt.hist([log10(x) for x in weights_list])
    plt.xlabel('log of weight magnitudes (log base 10)')
    plt.title('Log-Scale Histogram of Weights Learned by IDK on HUMOD')
    plt.savefig('humod_idk_l1_bce_weights_log10.jpg')

    weights_list.sort(reverse=True)
    print(weights_list[:10])

    # Get N biggest magnitude dims
    N = 7
    #print(weights > weights_list[N])
    torch.save(weights > weights_list[N], f'histogram/mask_{N}_humod_idk_l1_bce.pt')
